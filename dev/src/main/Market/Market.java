...
public class Market {
    ...
    public void addBargainPolicy(String userToken, String storeName, String productName, Double originalPrice) {
        getConnectedUserByToken(userToken).addBargainPolicy(getDomainStoreByName(storeName), productName,
                originalPrice);
    }
    ...
    public enum StatsType {
        Register, Login, Purchase, GuestVisitor, NonStaffVisitor, ManagerVisitor, OwnerVisitor, AdminVisitor
    }
    ...
    public Market(IPayment Psystem, ISupplying Ssystem, ConcurrentHashMap<String, User> users,
            ConcurrentHashMap<String, Store> stores, ConcurrentHashMap<LocalDate, SystemStats> stats,
    ...
    public boolean bidOnProduct(String userToken, String storeName, String productName, Double costumePrice,
        ...
        return user.bidOnProduct(store, productName, costumePrice, paymentInformation, supplyingInformation, Psystem,
                Ssystem);
    }
    ...
    public UserDTO DisconnectGuest(String user_token) {
        if (!connectedSessions.containsKey(user_token)) {
            Logger.getInstance().logBug("Market",
                    String.format("invalid user token attempted to disconnect %s", user_token));
    ...
    public UserDTO Login(String token, String userName, String password) {
        ...
        if (!connectedSessions.containsKey(token)) {
            Logger.getInstance().logBug("Market",
                    String.format("token %s isn't in the system and attempted to log in.", token));
    ...
    public List<ProductDTO> getProductsByAttributes(String productName, String category, String keyWord,
            Double productRating, Double storeRating, Double minPrice, Double maxPrice) {
    ...
    public boolean addProductToCart(String userToken, String storeName, String productName, int quantity) {
        ...
        if (!st.isProductAddable(productName))
            throw new IllegalArgumentException(
                    "Can't add this product to the cart - its policy doesn't allow to do so");
    ...
    private boolean allowOrDisallowPermission(String userToken, String managerName, String storeName,
            StorePermission permission, boolean shouldGrant) {
        Pair<User, Store> p = getConnectedUserAndStore(userToken, storeName);
    ...
    private void initialize(IPayment Psystem, ISupplying Isystem, String adminUsername, String adminPassword) {
        ...
        if (!membersByUserName.containsKey(adminUsername)) {
            ...
            Logger.getInstance().logEvent("Market",
                    String.format("Added Default system admin with username: %s", adminUsername));
        }
        setSsystem(Isystem);
        setPsystem(Psystem);
    }
    ...
    public void writeProductReview(String userToken, String productName, String storeName, String reviewDescription,
            double points) {
        User u = getConnectedUserByToken(userToken);
    ...
    public void writeStoreReview(String userToken, String storeName, String reviewDescription, double points)
            throws Exception {
        User u = getConnectedUserByToken(userToken);
        if (!u.getIsLoggedIn())
    ...
    public void sendComplaint(String userToken, String msg) throws Exception {
        ...
        if (getPurchaseHistory(userToken, user.getUserName()).isEmpty()) {
            throw new IllegalArgumentException(
                    "User has no purchase history. Cant send complaint without purchasing any product");
        }
    ...
    public void ApproveBid(String userToken, String storeName, String productName, String username) throws Exception {
        ...
        getConnectedUserByToken(userToken).ApproveBid(getDomainStoreByName(storeName), productName, user, Psystem,
                Ssystem);
    }
    ...
    public void CounterOfferBid(String userToken, String storeName, String productName, String username, Double offer)
            throws Exception {
        User user = membersByUserName.get(username);
    ...
    public int CreateSecretDiscount(String userToken, String store, LocalDate until, Double percent,
            String secretCode) {
        return getConnectedUserByToken(userToken).CreateSecretDiscount(getDomainStoreByName(store), until, percent,
                secretCode);
    }

    public int CreateConditionalDiscount(String userToken, String store, LocalDate until, Double percent, int condID) {
        return getConnectedUserByToken(userToken).CreateConditionalDiscount(getDomainStoreByName(store), until, percent,
                condID);
    }

    public int CreateMaximumCompositeDiscount(String userToken, String store, LocalDate until,
            List<Integer> discounts) {
        return getConnectedUserByToken(userToken).CreateMaximumCompositeDiscount(getDomainStoreByName(store), until,
                discounts);
    }

    public int CreatePlusCompositeDiscount(String userToken, String store, LocalDate until, List<Integer> discounts) {
        return getConnectedUserByToken(userToken).CreatePlusCompositeDiscount(getDomainStoreByName(store), until,
                discounts);
    }
    ...
    public int CreateBasketValueCondition(String userToken, String store, double requiredValue) {
        return getConnectedUserByToken(userToken).CreateBasketValueCondition(getDomainStoreByName(store),
                requiredValue);
    }

    public int CreateCategoryAmountCondition(String userToken, String store, String category, int amount) {
        return getConnectedUserByToken(userToken).CreateCategoryAmountCondition(getDomainStoreByName(store), category,
                amount);
    }

    public int CreateProductAmountCondition(String userToken, String store, String productName, int amount) {
        return getConnectedUserByToken(userToken).CreateProductAmountCondition(getDomainStoreByName(store), productName,
                amount);
    }
