...
public class ATuser2Requirements {
    ...
    @Test
    public void PurchaseWithDirectDiscount() {
        int amount = 5;
        service.addDirectDiscount(owner1token.getResult(), "MyStore1", "Coca Cola", LocalDate.MAX, 0.2);
    ...
    @Test
    public void PurchaseWithDirectDiscountPastTime() {
        int amount = 5;
        service.addDirectDiscount(owner1token.getResult(), "MyStore1", "Coca Cola", LocalDate.MIN, 0.2);
        service.addProductToCart(user1token.getResult(), "MyStore1", "Coca Cola", amount);
        Response<Boolean> r = service.purchaseCart(user1token.getResult(), pi, si);
        assertFalse(r.isError_occured());
        verify(mockPayment, times(1)).makePayment(pi, amount * CokePrice);
    ...
    @Test
    public void PurchaseWithSecretCodeDiscount() {
        int amount = 5;
        String secretCode = "Code";
        service.addSecretDiscount(owner1token.getResult(), "MyStore1", "Coca Cola", LocalDate.MAX, 0.2, secretCode);
        service.addProductToCart(user1token.getResult(), "MyStore1", "Coca Cola", amount);
        service.addDiscountPasswordToBasket(user1token.getResult(), "MyStore1", secretCode );
        Response<Boolean> r = service.purchaseCart(user1token.getResult(), pi, si);
        assertFalse(r.isError_occured());
        verify(mockPayment, times(1)).makePayment(pi, amount * CokePrice * 0.8);
        verify(mockSupplyer, times(1)).supply(any(SupplyingInformation.class), anyMapOf(Product.class, Integer.class));
    ...
    @Test
    public void PurchaseWithWrongSecretCodeDiscount() {
        int amount = 5;
        String secretCode = "Code";
        service.addSecretDiscount(owner1token.getResult(), "MyStore1", "Coca Cola", LocalDate.MAX, 0.2, secretCode);
        service.addProductToCart(user1token.getResult(), "MyStore1", "Coca Cola", amount);
        service.addDiscountPasswordToBasket(user1token.getResult(), "MyStore1", "Wrong code" );
        Response<Boolean> r = service.purchaseCart(user1token.getResult(), pi, si);
        assertFalse(r.isError_occured());
        verify(mockPayment, times(1)).makePayment(pi, amount * CokePrice);
        verify(mockSupplyer, times(1)).supply(any(SupplyingInformation.class), anyMapOf(Product.class, Integer.class));
    ...
    // 2 coke => 0.2
    // 3 cokes => 0.4
    private HashMap<HashMap<String, Integer>, Double> getRestrictions(){
        HashMap<HashMap<String, Integer>, Double> output = new HashMap<>();
        HashMap<String,Integer> rest1 = new HashMap<>();
        rest1.put("Coca Cola", 2);
        HashMap<String,Integer> rest2 = new HashMap<>();
        rest2.put("Coca Cola", 3);
        output.put(rest1, 0.2);
        output.put(rest2, 0.4);
        return output;
    }
